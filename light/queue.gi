...
InstallMethod( QueueCreate, "initialize a singly linked list queue",
  [ IsInt ],
  function(q)
    q := rec( first := fail, last := fail, size := 0 );
    Objectify(NewType(QueueFamily,IsSinglyLinkedListQueue and IsMutable),q);
    return q;
  end );
